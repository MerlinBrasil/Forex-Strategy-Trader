...
namespace Forex_Strategy_Trader
{
	...
	public class Accelerator_Oscillator : Indicator
	{
        ...
        public override void Calculate(SlotTypes slotType)
        {
            ...
            MAMethod  method    = (MAMethod )IndParam.ListParam[1].Index;
            BasePrice basePrice = (BasePrice)IndParam.ListParam[2].Index;
            int    slow  = (int)IndParam.NumParam[0].Value;
            int    fast  = (int)IndParam.NumParam[1].Value;
            int    accel = (int)IndParam.NumParam[2].Value;
            double level = IndParam.NumParam[3].Value;
            int    prev  = IndParam.CheckParam[0].Checked ? 1 : 0;

            // Calculation
            int      firstBar = slow + accel + 2;
			double[] MASlow = MovingAverage(slow, 0, method, Price(basePrice));
			double[] MAFast = MovingAverage(fast, 0, method, Price(basePrice));
			double[] AO = new double[Bars];
			double[] AC = new double[Bars];
            ...
            double[] AOMA = MovingAverage(accel, 0, method, AO);

            for (int bar = firstBar; bar < Bars; bar++)
            {
                AC[bar] = AO[bar] - AOMA[bar];
            }
            ...
            OscillatorLogic(firstBar, prev, AC, level, -level, ref Component[1], ref Component[2], indicatorLogic);
